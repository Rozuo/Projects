...
function Paddle:init(skin)
    ...
    -- dimensions that the paddle can take. We don't consider y since it is always constant
    self.dimensions = {['smallest'] = 32, ['medium'] = 64, ['large'] = 96, ['huge'] = 128}

    -- starting dimensions
    self.width = self.dimensions['medium']
    self.height = 16
...
function Paddle:changeDimensions()
    if(self.size == 1) then
        self.width = self.dimensions['smallest']
    elseif (self.size == 2) then
        self.width = self.dimensions['medium']
    elseif (self.size == 3) then
        self.width = self.dimensions['large']
    elseif(self.size == 4) then
        self.width = self.dimensions['huge']
    end
end