...
function Room:generateObjects()
    ...
    -- spawn pots at a max of 3 per room
...
function Room:update(dt)
    ...
    for i = #self.entities, 1, -1 do
        ...
        if entity.health <= 0 and not entity.dead then
            ...
            -- spawn in heart if we get the 1/8 chance
            if(math.random(3) == 1) then
                local heart = GameObject({
                    ...
                    states = {['unconsumed'] = {frame = 5}, ['consumed'] = {frame =5}},
    ...
    for k, object in pairs(self.objects) do
        ...
        if self.player:collides(object) then
            if(object.solid) then
                -- slight offsets so that we are not continually colliding with the pot
                if(self.player.direction == 'left') then
                    self.player.x = object.x + object.width + 0.99
                elseif(self.player.direction == 'right') then
                    self.player.x = object.x - object.width - 1
                elseif(self.player.direction == 'up') then
                    self.player.y = object.y + object.height - self.player.height/2 + 0.99
