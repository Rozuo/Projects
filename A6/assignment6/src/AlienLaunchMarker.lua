...
function AlienLaunchMarker:update(dt)
    ...
    elseif self.launched and love.keyboard.wasPressed('space') and not self.collided then -- split the aliens

        local alien = Alien(self.world, 'round', self.alien[1].body:getX(), self.alien[1].body:getY() + 40, 'Player')

        -- get the current linear velocity of the original alien and adjust it slightly to have a little differences in between the aliens
        local xVel, yVel = self.alien[1].body:getLinearVelocity()

        alien.body:setLinearVelocity(xVel + 30, yVel + 30)
        -- make the alien pretty bouncy
        alien.fixture:setRestitution(0.4)
        alien.body:setAngularDamping(1)
        table.insert(self.alien, alien)
        ...
        alien = Alien(self.world, 'round', self.alien[1].body:getX(), self.alien[1].body:getY() - 40, 'Player')
        alien.body:setLinearVelocity(xVel - 30, yVel - 30)
        -- make the alien pretty bouncy
        alien.fixture:setRestitution(0.4)
        alien.body:setAngularDamping(1)
