...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        local xPos1, yPos1 = self.launchMarker.alien[1].body:getPosition()
        local xVel1, yVel1 = self.launchMarker.alien[1].body:getLinearVelocity()

        -- if we fired our alien to the left or it's almost done rolling, respawn
        if xPos1 < 0 or (math.abs(xVel1) + math.abs(yVel1) < 1.5) then
            self.launchMarker.alien[1].body:destroy()
            self.launchMarker = AlienLaunchMarker(self.world)
            self.launchMarker.collided = false
